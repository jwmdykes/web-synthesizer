...
function WhiteKey(props: {
  ...
  return (
    <>
      <div className='w-full relative' onMouseDown={props.mouseDownCallback}  onMouseUp={props.mouseUpCallback}>
...
export default function Piano(props: {mouseDownCallback: MouseEventHandler, mouseUpCallback: MouseEventHandler}) {
        ...
        <div className='h-full'>
          <div className='flex flex-row h-full overflow-hidden'>
            {whiteKeys.map((key, index) => {
              return (
                <>
                  <WhiteKey letter={key} key={key} mouseDownCallback={props.mouseDownCallback} mouseUpCallback={props.mouseUpCallback}>
                    {
                        (key !== 'B' && key !== 'E' && index !== whiteKeys.length - 1) ? <BlackKey letter={key + '#'} mouseDownCallback={props.mouseDownCallback} mouseUpCallback={props.mouseUpCallback}/> : null
                    }
                  </WhiteKey>
                </>
              );
